...
#if defined(USE_IMGUI_WINDOWS) && defined(USE_IMGUI_DX12) && defined(USE_IMGUI)
#include "imgui.h"
...
// Create application window
    ...
    if (!CreateDeviceD3D(hwnd))
    {
        ...
        critical_error("Failed to initalize Direct3D");
    }
...
void WayfarerGUI::cleanup()
{
    if (!this->done)
    {
        critical_error("Called cleanup when we were not done rendering GUI");
    }
    else
    {
        WaitForLastSubmittedFrame();
...
#else //  defined(USE_IMGUI_WINDOWS) && defined(USE_IMGUI_DX12) && defined(USE_IMGUI)

void init()
{
    critical_error("Can't initialize GUI. Compiled without a specified GUI framework/backend\n");
}

void WayfarerGUI::cleanup()
{
    critical_error("Can't cleanup GUI. Compiled without a specified GUI framework/backend\n");
}

bool WayfarerGUI::update_gui()
{
    critical_error("Can't update GUI. Compiled without a specified GUI framework/backend\n");
}
#endif // defined(USE_IMGUI_WINDOWS) && defined(USE_IMGUI_DX12) && defined(USE_IMGUI)

WayfarerGUI::WayfarerGUI()
{
    init();
}
